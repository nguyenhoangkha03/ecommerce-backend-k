...
const getCart = async (req, res, next) => {
  try {
    ...
    const subtotal = cartItemsWithWarranties.reduce((sum, item) => {
      // Ưu tiên price đã lưu trong CartItem (chính xác nhất), 
      // sau đó mới fallback đến ProductVariant hoặc Product price
      const price = item.price || (item.ProductVariant
...
const addToCart = async (req, res, next) => {
  ...
  try {
    ...
    // Validate variant if provided, or get default variant for variant products
    let variant = null;
    if (variantId) {
      ...
      // If no variantId provided but product is variant product, get the first available variant
      if (product.isVariantProduct) {
        variant = await ProductVariant.findOne({
          where: { productId },
          order: [['createdAt', 'ASC']], // Get first variant as default
        });
        
        if (!variant) {
          throw new AppError('Sản phẩm không có biến thể khả dụng', 404);
        }
