...
const authenticate = async (req, res, next) => {
  try {
    ...
    // Check if email is verified - Allow Facebook users to continue regardless
    // For local users, only check verification if they don't have a verification token
    // (verification token indicates they are updating their email)
    if (!user.isEmailVerified && user.provider === 'local' && !user.verificationToken) {
      return next(
